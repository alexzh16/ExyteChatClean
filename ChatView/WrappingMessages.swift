   ...
   static private func wrapSectionMessages(_ messages: [Message], chatType: ChatType, replyMode: ReplyMode, isFirstSection: Bool, isLastSection: Bool) -> [MessageRow] {
      messages
         ...
         .map {(index, message) in
            ...
            if replyMode == .quote {
               return MessageRow(message: message, positionInUserGroup: position, commentsPosition: nil)
            }
            ...
            return MessageRow(message: message, positionInUserGroup: position, commentsPosition: commentsPosition)
         }
         .reversed()
   }
}
